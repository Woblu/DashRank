  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(apiEndpoint,
        { levelName, difficulty, attempts, videoUrl, rawFootageLink },
        { headers: { Authorization: `Bearer ${token}` } }
      );
          ...
          {loading ? <p className="text-center text-gray-400">Loading records...</p> : records.length > 0 ? records.map(record => (
            <div key={record.id} className="flex justify-between items-center bg-gray-900 p-3 rounded-lg">
              <div className="flex-grow">
                <p className="font-bold text-lg text-cyan-400">{record.levelName}</p>
                <p className="text-sm text-gray-400">{record.difficulty.replace('_', ' ')} Demon {record.attempts ? `- ${record.attempts} attempts` : ''}</p>
                <div className="flex items-center gap-4 mt-2 text-xs">
                  <a href={record.videoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-gray-300 hover:text-cyan-400 transition-colors">
                    <Film className="w-4 h-4" /> Video Proof
                  </a>
                  {record.rawFootageLink && (
                    <a href={record.rawFootageLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-gray-300 hover:text-cyan-400 transition-colors">
                      <LinkIcon className="w-4 h-4" /> Raw Footage
                    </a>
                  )}
                </div>
              </div>
              <button onClick={() => handleDelete(record.id)} className="p-2 ml-2 text-red-500 hover:bg-red-500/20 rounded-full transition-colors">
