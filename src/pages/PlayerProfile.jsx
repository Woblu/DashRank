...
// Import ALL static lists needed
...
// import futureList from '../data/future-list.json'; // Add if needed
...
export default function PlayerProfile() {
  ...
  useEffect(() => {
    const fetchAndProcessData = async () => {
      ...
      // --- 2. Process Static JSONs Separately for Completed/Verified Lists ---
      const tempVerifiedByList = {};
      const tempBeatenByList = {};
      const verifiedLevelNames = new Set(); // Keep track of verified level names

      console.log("[PlayerProfile] Processing static lists for VERIFIED levels...");
          ...
          for (const level of staticLevels) {
              ...
              if (level.verifier?.toLowerCase() === canonicalNameLower) {
                  ...
                  if (!tempVerifiedByList[listType].some(l => l.name?.toLowerCase() === levelNameLower)) {
                      tempVerifiedByList[listType].push({ ...level, listType, levelName: level.name });
      ...
      console.log("[PlayerProfile] Processing static lists for COMPLETED levels...");
       for (const listType in allStaticLists) {
          const staticLevels = allStaticLists[listType];
          if (!Array.isArray(staticLevels)) continue;
           ...
           for (const level of staticLevels) {
               ...
               if (Array.isArray(level.records)) {
                    // Check if *any* record matches the player name and 100%
                    if(level.records.some(r => r.username?.toLowerCase() === canonicalNameLower && r.percent === 100)) {
                        isCompleted = true;
                    }
               }

               // [FIX] Add to completed list ONLY if completed AND NOT verified by this player
               if (isCompleted && !verifiedLevelNames.has(levelNameLower)) {
                   console.log(` -> Found completed (not verified): ${level.name} (${listType})`);
                   ...
                   if (!tempBeatenByList[listType].some(l => l.name?.toLowerCase() === levelNameLower)) {
                       tempBeatenByList[listType].push({ ...level, listType, levelName: level.name });
                   }
               }
           }
       }
      ...
      console.log("[PlayerProfile] Grouped Completed Levels (final):", tempBeatenByList);
      console.log("[PlayerProfile] Grouped Verified Levels (final):", tempVerifiedByList);
      ...
      if (apiStats?.hardestDemonName) {
           ...
           if (detailsFromStatic) {
                hardestDemonDetails = {
                    ...detailsFromStatic, // Use static details (like levelId, listType)
                    // [FIX] ALWAYS use the placement from the API stats, as it's the most current
                    placement: apiStats.hardestDemonPlacement ?? detailsFromStatic.placement,
                    levelName: apiStats.hardestDemonName // Ensure correct name casing from API
                };
                console.log("[PlayerProfile] Found details for hardest demon, prioritizing API placement:", hardestDemonDetails);

           } else {
                // Fallback if details lookup fails
                 hardestDemonDetails = {
                     levelName: apiStats.hardestDemonName,
                     placement: apiStats.hardestDemonPlacement || '?',
                     listType: 'main', // Assume main if no details found
                     id: null, // Indicate details are incomplete
                     levelId: null
                 };
                  console.warn(`[PlayerProfile] Could not find static details for hardest demon "${apiStats.hardestDemonName}". Using API data only.`);
           }
      } else {
           console.log("[PlayerProfile] No Hardest Demon info from API.");
           // Optional: Calculate hardest dynamically ONLY IF API fails?
           // Find hardest from combined beaten/verified main list if API didn't provide
           let calculatedHardest = null;
           let minPlacement = Infinity;
           [...(tempBeatenByList.main || []), ...(tempVerifiedByList.main || [])].forEach(level => {
               if (typeof level.placement === 'number' && level.placement < minPlacement) {
                   minPlacement = level.placement;
                   calculatedHardest = level;
               }
           });
           if (calculatedHardest) {
               console.log(`[PlayerProfile] Hardest from API missing, calculated from static: ${calculatedHardest.name} (#${calculatedHardest.placement})`);
               hardestDemonDetails = calculatedHardest; // Use the dynamically calculated one as fallback
           }
      }
      ...
      // --- 4. Final Check and Set State ---
       ...
       if (!apiStats && Object.keys(tempBeatenByList).length === 0 && Object.keys(tempVerifiedByList).length === 0) {
           ...
           setProfileData({
               ...
               beatenByList: tempBeatenByList, // From static JSONs
               verifiedByList: tempVerifiedByList, // From static JSONs
               hardestDemon: hardestDemonDetails, // Use processed/calculated hardest demon
               loading: false,
        ...
        <div className="bg-gray-100 dark:bg-gray-800 p-6 rounded-xl shadow-lg">
          ...
          <div className="text-center border-t border-gray-300 dark:border-gray-600 pt-4 space-y-2 text-gray-800 dark:text-gray-200">
            ...
            {hardestDemon && hardestDemon.levelName ? ( // Check if hardestDemon object has a name
              ...
              // Show N/A only if hardestDemon calculation failed or no main list completions found
            ...
            {/* Counts - Calculated from static JSON processing */}
             ...
             {totalVerified > 0 && (
              ...
              {/* You can add counts per list here if desired */}
          </div>
        </div>
