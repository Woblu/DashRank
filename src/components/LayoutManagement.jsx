            ...
            <div>
              ...
              <div className="space-y-4">
                {parts.length > 0 ? parts.map(part => {
                  const canManageStatus = user?.id === layoutCreatorId || user?.id === part.assigneeId;
                  return (
                    <div key={part.id} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center p-4 bg-gray-700/50 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <select value={part.assigneeId || ''} onChange={(e) => handleAssignPart(part.id, e.target.value)} disabled={user?.id !== layoutCreatorId} className="w-full p-2 rounded-lg border border-gray-600 bg-gray-800 text-gray-200 disabled:opacity-50">
