    ...
    >
      ...
      <div className="flex flex-col flex-grow text-center sm:text-left">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          {listType === 'progression' ? 'Your Record' : `${t('by')} ${level.creator}`}
        </p>
      </div>

      {/* Conditionally render Edit/Delete buttons ONLY for the progression tracker */}
      {listType === 'progression' && (
        <div className="flex flex-col sm:flex-row gap-1">
          <button 
            type="button" 
            onClick={(e) => {
              e.stopPropagation(); // Stop the card's click handler from firing
              onEdit(level);
            }} 
